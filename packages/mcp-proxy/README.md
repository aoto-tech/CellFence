...
- `npx cellfence@0.4.0 init` generates `cellfence.manifest.json` with `schemaVersion: "cellfence.manifest.v1"`, optional `governance`, and `cells[]` containing `id`, `ownedPaths`, `publicEntry`, `publicSymbols`, `consumes`, and `producesArtifacts`.
