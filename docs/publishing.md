...
```bash
git tag -s v0.4.0 -m "CellFence v0.4.0"
...
```bash
gh workflow run npm-publish.yml --repo OWNER/REPOSITORY --ref v0.4.0 -f dry_run=true
```
...
```bash
gh workflow run npm-publish.yml --repo OWNER/REPOSITORY --ref v0.4.0 -f dry_run=false -f confirm_publish="publish 0.4.0"
```
...
1. Create a signed tag, for example `v0.4.0`.
