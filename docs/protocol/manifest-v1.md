...
Machine-readable findings explain observed facts, applied manifest or baseline contracts, the judgment made from those contracts, and items CellFence did not verify. Normal diagnostics do not include remediation patches, replacement import targets, manifest additions, baseline updates, or rule-disabling steps as suggested resolutions. The deprecated `suggestedResolutions` field remains in compatibility types for older callers and plugins, but emitted CellFence diagnostics omit it. A diagnostic may expose contract information such as public entries, owned paths, allowed dependencies, and actual baseline diffs; that information is not a guarantee that any particular implementation change is correct.
